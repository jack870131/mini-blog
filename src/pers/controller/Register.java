...
@WebServlet(
		...
		initParams={
				...
				@WebInitParam(name = "ERROR_PATH", value = "/WEB-INF/jsp/register.jsp") //註冊網頁路徑
		})
public class Register extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
    	request.getRequestDispatcher(getInitParameter("FORM_PATH")).forward(request, response); //直接轉發註冊申請網頁
    }
    ...
    protected void doPost(
        ...
        if(errors.isEmpty()) {
            ...
            if(optionalAcct.isPresent()) {
            	emailService.validationLink(optionalAcct.get()); //傳送驗證郵件
            } else {
            	emailService.failedRegistration(username, email); //傳送申請失敗郵件
            }
            
            userService.tryCreateUser(email, username, password);
        } else {
            path = getInitParameter("FORM_PATH"); //設定註冊申請失敗時的轉發網頁
