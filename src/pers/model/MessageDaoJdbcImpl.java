...
public class MessageDaoJdbcImpl implements MessageDao {
	...
	public MessageDaoJdbcImpl(DataSource dataSouce) {
		this.dataSource = dataSource;
	}

	@Override
	public List<Message> messageBy(String username) {
		try(Connection conn = dataSource.getConnection();
			PreparedStatement stmt = conn.prepareStatement(
				"SELECT * FROM t_message WHERE name = ?")) {
	...
	public void createMessage(Message message) {
		try(Connection conn = dataSource.getConnection();
			PreparedStatement stmt = conn.prepareStatement(
				"INSERT INTO t_message(name, time, blabla) VALUES(?, ?, ?)")) {
	...
	public void deleteMessageBy(String username, String millis) {
		try(Connection conn = dataSource.getConnection();
				PreparedStatement stmt = conn.prepareStatement(
						"DELETE FROM t_message WHERE name = ? AND time = ?")) {
			stmt.setString(1, username);
	...
	@Override
	public List<Message> newestMessage(int n) {
		try(Connection conn = dataSource.getConnection();
				PreparedStatement stmt = conn.prepareStatement(
